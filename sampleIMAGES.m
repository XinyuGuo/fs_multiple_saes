...
%image_index = randi(10,numpatches,1);  % random integer between 1 and 10, uniform distribution
%patch_start = randi(505,numpatches,1); % dimension start should between 1 and 505 so that
                                       % dimension end can be between
                                       % 8 and 512                                       
%for theImage  = 1:numpatches
%    thisPatch = IMAGES(patch_start:patch_start+7,patch_start:patch_start+7,image_index(theImage,1));
%    patches(:,theImage) = reshape(thisPatch,64,1);
%end 
